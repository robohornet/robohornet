...
(function() {
  ...
  _p.init = function() {
    ...
    var self = this;
    this.testsContainer_.addEventListener('click', function(e) {
  ...
  _p.toggle_ = function(e) {
    for (var benchmark, i = 0; benchmark = this.benchmarks_[i]; i++) {
      var identifier = 'benchmark-' + benchmark.index;
      benchmark.enabled = document.getElementById(identifier +
          '-toggle').checked;

      var el = document.getElementById(identifier + '-runs').parentNode;
      el.className = 'details-container ' + (benchmark.enabled ? 'enabled' :
          'disabled');
    }
  };
  ...
  _p.removeListeners_ = function(benchmark) {
    ...
    this.testFrame_.removeEventListener('error', benchmark.errorCallback_,
        false);
  ...
  _p.setResults_ = function(benchmark, results) {
    ...
    var score = benchmark.baselineTime * benchmark.computedWeight /
        accumulatedMean;
